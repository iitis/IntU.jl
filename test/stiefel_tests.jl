...
@testset "Stiefel Manifold V_k(C^d)" begin
    ...
    @testset "Normalization" begin
        ...
        V = [Symbolics.variable(Symbol("V_$(i)_$(j)"), T=Complex{Num}) for i=1:d_fixed, j=1:k_fixed]
    ...
    @testset "Symbolic d Normalization" begin
        ...
        V_small = [Symbolics.variable(Symbol("Vs_$(i)_$(j)"), T=Complex{Num}) for i=1:2, j=1:2]
    ...
    @testset "Consistency with dPsi (k=1)" begin
     ...
     @testset "Consistency with dU (k=d)" begin
        ...
        d_fixed = 3
        # U must be complex
        U = [Symbolics.variable(Symbol("U_$(i)_$(j)"), T=Complex{Num}) for i=1:d_fixed, j=1:d_fixed]
