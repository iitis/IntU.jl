...
unitary_abs2_moment(d, k::Int) = factorial(k) / prod(d + i for i in 0:k-1)

# U(d): for distinct components in a row, (|U_1j|^2) are Dirichlet(1,...,1)
# E[∏_{j=1}^m |U_1j|^2] = 1 / (d)(d+1)...(d+m-1)
unitary_row_product(d, m::Int) = 1 / prod(d + i for i in 0:m-1)

# U(d): E[(|U_11|^2)^2 (|U_12|^2)^2] under Dirichlet(1,...,1)
unitary_row_square_square(d) = (2*2) / prod(d + i for i in 0:3)  # 4 / d(d+1)(d+2)(d+3)

# O(d): if O_11^2 ~ Beta(1/2, (d-1)/2), then E[O_11^(2k)] = (2k-1)!! / d(d+2)...(d+2k-2)
orth_entry_even_moment(d, k::Int) = doublefactorial_odd(2k-1) / prod(d + 2i for i in 0:k-1)

# O(d): squared coordinates Dirichlet(1/2,...,1/2)
# E[∏_{j=1}^m O_1j^2] = 1 / d(d+2)...(d+2m-2)
orth_row_product(d, m::Int) = 1 / prod(d + 2i for i in 0:m-1)

# O(d): E[(O_11^2)^2 (O_12^2)^2] with Dirichlet(1/2,...,1/2) gives 9 / d(d+2)(d+4)(d+6)
orth_row_square_square(d) = 9 / prod(d + 2i for i in 0:3)
...
U_cases = [
    ...
    ("U_row_prod4__∏|U1j|^2", prod(abs2(U[1,j]) for j in 1:4), unitary_row_product(d, 4)),
    ("U_row_sq_sq__|U11|^4|U12|^4", (abs2(U[1,1])^2) * (abs2(U[1,2])^2), unitary_row_square_square(d)),
]
...
for (name, expr, expected) in U_cases
    ...
    for dv in dvals_symbolic
        ...
        numeric_checks[string(dv)] = Dict("got" => safe_string(g), "expected" => safe_string(e), "ok" => (g == e))
    end
    ...
    println("  median: $(bm["median_ms"]) ms, mem: $(bm["memory_bytes"]) B, allocs: $(bm["allocs"])")
    println()
...
O_cases = [
    ...
    ("O_row_prod4__∏O1j^2", prod(O[1,j]^2 for j in 1:4), 4),
...
for dnum in dnums
    ...
    expr = Obig[1,1]^10
    expected = Float64((doublefactorial_odd(9)) // (dnum*(dnum+2)*(dnum+4)*(dnum+6)*(dnum+8)))  # 945/...
...
for N in Nvals
    ...
    println("  ok       : $okC")
    println("  median: $(bmC["median_ms"]) ms, mem: $(bmC["memory_bytes"]) B, allocs: $(bmC["allocs"])")
    println()
