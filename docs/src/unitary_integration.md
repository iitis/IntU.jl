...
where:
- $S_n$ is the symmetric group of degree $n$ (permutations of $n$ elements).
- $\delta_{i, k_\sigma} = \prod_{m=1}^n \delta_{i_m, k_{\sigma(m)}}$ (contraction of row indices).
- $\delta_{j, l_\tau} = \prod_{m=1}^n \delta_{j_m, l_{\tau(m)}}$ (contraction of column indices).
...
-   **Computational Complexity**: The sum involves $(n!)^2$ terms. While
    optimized to group cycles, integrals with high degrees ($n > 6$) can become
    computationally expensive.
-   **Ambiguity with Conjugates**: Ensure you use `conj()` correctly. In Julia, `A'` is the conjugate transpose. IntU.jl handles `conj(U[i,j])` as $\bar{U}_{ij}$.
