...
res_coe = simplify(integrate(expr_coe, m_coe))
println("E[|S_11|^2] = ", res_coe, " (Expected: 2/(d+1))")

# Moment: E[|S_12|^2] = 1/(d+1)
expr_coe_12 = S[1,2] * conj(S[1,2])
res_coe_12 = simplify(integrate(expr_coe_12, m_coe))
...
res_cse = simplify(integrate(expr_cse, m_cse))
println("E[|S_11|^2] = ", res_cse, " (Expected: 1/(d-1))")

# Moment: E[|S_12|^2] = 0 (off-diagonal elements in symplectic pair blocks behave differently?)
# Actually for CSE, E[S_{ij} S*_{kl}] involves symplectic deltas.
# Let's check a cross term: E[S_12 S*_12]
expr_cse_12 = S_cse[1,2] * conj(S_cse[1,2])
res_cse_12 = simplify(integrate(expr_cse_12, m_cse))
println("E[|S_12|^2] = ", res_cse_12)
