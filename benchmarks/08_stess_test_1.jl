...
function test_orthogonal(N::Int)
    ...
    if N >= 2
        run_example("O4: ∫ O₁₁² O₁₂² dO = 1/(d(d+2))", O[1,1]^2 * O[1,2]^2, μO, 1/(d*(d+2)); subs=subsO)
        run_example("O5: ∫ O₁₁² O₂₂² dO = (d+1)/(d(d-1)(d+2))", O[1,1]^2 * O[2,2]^2, μO, (d+1)/(d*(d-1)*(d+2)); subs=subsO)
        run_example("O6: ∫ O₁₁ O₁₂ O₂₁ O₂₂ dO = -1/(d(d-1)(d+2))", O[1,1]*O[1,2]*O[2,1]*O[2,2], μO, -1/(d*(d-1)*(d+2)); subs=subsO)
    end
end
...
function test_application()
    ...
    purity = zero(Num)
    for a in 1:nA, ap in 1:nA, b in 1:nB, bp in 1:nB
