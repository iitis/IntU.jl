...
% Engine row
\coordinate (engine_row_anchor) at ($(dispatch.south)+(0,-2.1cm)$);
...
\coordinate (trace_lane) at ($(dispatch.east)+(1.25cm,0)$);
\coordinate (fanout_hub) at ($(dispatch.south)+(0,-0.5cm)$);
\coordinate (outer_lane_top) at ($(library.east)+(1.1cm,0)$);
...
% Trace merge to dispatch via dedicated right-side lane
\draw[arr] (graph.south) |- (trace_lane) |- (dispatch.east);
...
% Dispatch fan-out through shared manifold
\draw[arr] (dispatch.south) -- (fanout_hub);
\draw[arr] (fanout_hub) |- (wg_u.north);
\draw[arr] (fanout_hub) |- (wg_o.north);
\draw[arr] (fanout_hub) |- (wick.north);
\draw[arr] (fanout_hub) |- (comb.north);
