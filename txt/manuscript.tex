...
Beyond compact groups, the package provides comprehensive support for the beta
ensembles of random matrix theory (GUE, GOE, GSE, GinUE, GinOE, GinSE) via
Wick's theorem, as well as circular ensembles and unitary $t$-designs. This
diverse ensemble support, combined with specialized utilities for quantum
information metrics like purity and fidelity, makes \texttt{IntU.jl} a versatile
...
\begin{itemize}
    ...
    computing averages of purity, fidelity, and partial traces of symbolic
    subsystems.
...
\subsection{Special Unitary group $SU(d)$}
The Special Unitary group $SU(d)$ consists of unitary matrices with unit
determinant. In the stable range regime (large $d$), integration over $SU(d)$
coincides with $U(d)$ for balanced polynomials (where the number of $U$ and
$\bar{U}$ indices matches). For unbalanced polynomials, which would typically
involve $\epsilon$-tensor contractions at specific finite dimensions, the stable
range integral vanishes. \texttt{IntU.jl} currently supports this stable range
behavior, treating $SU(d)$ integrals as equivalent to $U(d)$ for balanced
moments and zero otherwise.
...
\begin{itemize}
    ...
    \item \textbf{Special unitary group}: The Haar measure on $SU(d)$, accessed
    via \texttt{dSU(U, d)}. This measure enforces the $U(d)$ integration logic
    for balanced moments while strictly annihilating unbalanced terms,
    consistent with the large-$d$ limit.
...
\begin{itemize}
    \item \textbf{GinUE}: $\langle G_{ij} \bar{G}_{kl} \rangle = \delta_{ik}
    \delta_{jl}$. Only contractions between $G$ and its conjugate $\bar{G}$ are
    non-vanishing.
...
\begin{itemize}
    ...
    transpose). The integration is similarly performed by mapping to $U(2d)$
    integrals.
\end{itemize}
These measures are accessed via \texttt{dCOE(S, d)}, \texttt{dCSE(S, d)}, and
\texttt{dCUE(U, d)} (alias for \texttt{dU}).
...
measure_coe = dCOE(S, d)
res_coe = integrate(abs(S[1,1])^2, measure_coe)
# Output: 2/(d+1)

# Circular Symplectic Ensemble (S is self-dual)
measure_cse = dCSE(S, d)
res_cse = integrate(abs(S[1,1])^2, measure_cse)
# Output: 1/(d-1)
\end{lstlisting}
...
For the special unitary group $SU(d)$, the integration behaves similarly to
$U(d)$ for balanced moments but differs for unbalanced ones.
\begin{lstlisting}[language=Julia]
measure = dSU(U, d)
...
# A constant tensor connecting the indices
A = randomITensor(j, i)

# Integrate the network over U(2)
result = integrate([U, A], dU(2))
# result is a scalar ITensor with value 1/2 * tr(A)
\end{lstlisting}
...
\begin{itemize}
    ...
    between $V$ and $\bar{V}$ factors. This specialized engine avoids Weingarten
    ...
    dimension calls, ensuring maximizing reuse across different partition
...
\begin{enumerate}
    ...
    $d$ is computationally demanding, but optimization strategies significantly reduce this cost.
    For example, computing $\int |U_{11}|^{10} dU$ symbolically
    took approximately 46 ms, achieving performance comparable to fixed $d=50$ numeric integration ($47$ ms).
    This efficiency leverages the grouping of identical cycle structures in the Weingarten sum.
    ...
    symbolic integration of $|U_{11}|^6$ completes in approximately 4 ms, and
    ...
    comparable to unitary ones in the optimized regime. While the number of pair partitions grows
    faster than permutations asymptotically, the overhead for moderate degrees is similar
    due to efficient grouping strategies.
    ...
    the unitary group, completes in approximately 81.1 ms, demonstrating the
    tool's applicability to quantum information tasks.
    \item \textbf{Tensor network scaling}: The graphical engine scales efficiently
    with both the degree $k$ and dimension $d$. As shown in Table
    \ref{tab:itensor_benchmarks}, for a loop network of Haar unitaries, the
    median time increases from 5.5 $\mu\text{s}$ at $k=1$ to 32 ms at $k=4$ for
    fixed $d=2$. Dimension scaling for $k=2$ remains sub-second up to $d=50$,
    reaching 1.3 s at $d=100$. For higher degrees like $k=3$, the overhead
    increases more rapidly with $d$, with $d=30$ taking 16.8 s, reflecting the
    ...
    10$^\mathrm{th}$ moment $P_{11}^{10}$ for $d=100$ takes only 65.7 ms. This
...
\hline
Unitary & $|U_{11}|^6$ & Symbolic & 2.3 \\
Unitary & $|U_{11}|^8$ & Symbolic & 4.7 \\
Unitary & $|U_{11}|^{10}$ & Symbolic & 12.5 \\
Unitary & $|U_{11}|^{10}$ & $d=10$ & 5.2 \\
Unitary & $|U_{11}|^{10}$ & $d=50$ & 46.8 \\
\hline
Orthogonal & $O_{11}^6$ & $d=10$ & 0.2 \\
Orthogonal & $O_{11}^8$ & $d=20$ & 1.1 \\
Orthogonal & $O_{11}^{10}$ & $d=20$ & 80.7 \\
Orthogonal & $O_{11}^{10}$ & $d=50$ & 93.3 \\
\hline
Symplectic & $|S_{11}|^8$ & $d=10$ & 1.8 \\
Symplectic & $|S_{11}|^{10}$ & $d=10$ & 4.9 \\
Symplectic & $|S_{11}|^{10}$ & $d=20$ & 9.6 \\
\hline
GinUE & $\mathrm{tr}(GG^\dagger)$ & Symbolic & 0.01 \\
GinUE & $\mathrm{tr}(GG^\dagger)$ & $d=4$ & 5.8 \\
\hline
Circ. Orthogonal & $|S_{11}|^2$ & Symbolic & 1.6 \\
Circ. Orthogonal & $|S_{11}|^4$ & Symbolic & 7.8 \\
Circ. Orthogonal & $|S_{11}|^6$ & Symbolic & 55.7 \\
Circ. Symplectic & $|S_{11}|^2$ & Symbolic & 1.6 \\
Circ. Symplectic & $|S_{11}|^4$ & Symbolic & 8.6 \\
...
51 ms) highlights the library's utility for routine quantum information tasks,
...
high-degree moments such as $|S_{11}|^6$ are computed in under 130~ms.
Simultaneously, the symbolic engine proves
capable of handling high-degree moments that are intractable by hand. The
caching mechanism ensures that subsequent calls with the same parameters are
...
Degree $k$ & 4 & 2 & 31.6 \\
\hline
Dimension $d$ ($k=2$) & 2 & 2 & 0.101 \\
Dimension $d$ ($k=2$) & 2 & 10 & 0.171 \\
Dimension $d$ ($k=2$) & 2 & 50 & 44.4 \\
Dimension $d$ ($k=2$) & 2 & 100 & 1092 \\
\hline
Dimension $d$ ($k=3$) & 3 & 2 & 2.11 \\
Dimension $d$ ($k=3$) & 3 & 10 & 26.5 \\
Dimension $d$ ($k=3$) & 3 & 20 & 1936 \\
Dimension $d$ ($k=3$) & 3 & 30 & 16607 \\
\hline
Orthogonal & 6 & 3 & 512.5 \\
\hline
\end{tabular}
\end{table}
