...
ensembles (CUE, COE, CSE), Ginibre ensembles (GinUE, GinOE, GinSE), and Gaussian ensembles (GUE, GOE, GSE), as well as
random pure quantum states, diagonal unitary matrices, and unitary $t$-designs.
Unlike existing tools that often rely on proprietary engines or lack support for
...
ensembles of random matrix theory (GUE, GOE, GSE, GinUE, GinOE, GinSE) via Wick's theorem, as well as
circular ensembles and unitary $t$-designs. This diverse ensemble support,
combined with specialized utilities for quantum information metrics like purity
and fidelity, makes \texttt{IntU.jl} a versatile environment for both
theoretical derivations and complex numerical experiments.
...
\begin{itemize}
    ...
    \item \textbf{Gaussian and Ginibre ensembles}: Support for the Gaussian unitary (GUE),
    orthogonal (GOE), and symplectic (GSE) ensembles, as well as the complex (GinUE), 
    real (GinOE), and symplectic (GinSE) Ginibre ensembles. These rely on Wick's 
    theorem for integration rather than Weingarten calculus.
...
Moving beyond Hermitian matrices, \texttt{IntU.jl} supports the Ginibre ensembles
(GinUE, GinOE, GinSE) \cite{ginibre1965statistical}, which consist of
...
\begin{itemize}
    \item \textbf{GinUE}: $\langle G_{ij} \bar{G}_{kl} \rangle = \delta_{ik} \delta_{jl}$. Only contractions between $G$ and its conjugate $\bar{G}$ are non-vanishing.
    \item \textbf{GinOE}: $\langle G_{ij} G_{kl} \rangle = \delta_{ik} \delta_{jl}$ for real entries.
...
matrix properties. We compute the second moment $\langle \mathrm{tr}(G G^\dagger) \rangle$:
\begin{lstlisting}[language=Julia]
@variables d
Gs = SymbolicMatrix(:G)
measure = dGinUE(Gs, d)
